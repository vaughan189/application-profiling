...
app.get("/api/tick", (req, res) => {
  arr.push({ name: "Vaughan" });
  Promise.resolve(
    "asynchronous flow will make our stacktrace more realistic".repeat(
      HOW_OBVIOUS_THE_FLAME_GRAPH_SHOULD_BE_ON_SCALE_1_TO_100
    )
  )
    .then((text) => {
      const randomText = Math.random()
        .toString(32)
        .repeat(HOW_OBVIOUS_THE_FLAME_GRAPH_SHOULD_BE_ON_SCALE_1_TO_100);
